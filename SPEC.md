...
- Each downstream child: spawn, health = initialized handshake completed. **Child environment (normative):** child env = parent env overlaid with the per-server `env` from config. Document the consequence plainly in the README: anything in Loom's own environment is visible to every child — put per-server secrets in per-server config `env`, not in the shell that launches Loom.
...
```jsonc
{
  "servers": [ { "name": "str (^[a-z][a-z0-9_]{0,31}$, unique, `loom` rejected as reserved)", "command": "str", "args": ["str"], "env": {"K":"V"} } ],
